...
rcbd_means_all <-  emmeans::emmeans(rcbd_mod, ~nrate_kgha:krate_kgha)
...
ggplot(mapping = aes(fill = trtname))+
  geom_boxplot(data = rcbd_dfw, aes(x = trtname, y= yield_kgha),
               alpha =.8,
               )+
  geom_jitter(data = rcbd_dfw, aes(x= trtname, y= yield_kgha), shape = 21, alpha = .6, size =3)+
  geom_label(data = rcbd_cld_selected,
             aes(x= trtname, y=emmean, label=letter), fill = "white")+
